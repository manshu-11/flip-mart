...
const ProductList = () => {
  ...
  useEffect(() => {
    ...
    let filterData = productData.filter((ele) => {
      ...
      if (filterPriceRange) {
        let getCurrencyPrice = currencyStatus
          ? Math.ceil(ele.price)
          : Math.ceil(ele.price) * Math.ceil(exchangeRate);
        matchesPrice =
          getCurrencyPrice >= +filterPriceRange[0] &&
          getCurrencyPrice <= +filterPriceRange[1];
      }
            ...
            return (
              <div key={prod.id}>
                <Suspense fallback={<ProductSkeleton />}>
                  <Product productData={prod} currencyStatus={currencyStatus} />
                </Suspense>
              </div>
            );
          })
