...
const PurchaseOrder = () => {
	...
	const fnNextActivity = () => {
		setCurrent((current) => {
			console.log(stepperData[current - 1].stepInfo);
	...
	return (
		userDetails ? <div className='purchase-order'>
				...
				<div className="steps">
					{
						stepperData.map(function (stepper, index) {
							return (<div className="step-box" key={stepper.stepInfo}>
								<div className={`step-no  ${current === (index + 1) && !isComplete ? "active" : ""} ${current > (index + 1) || isComplete ? "complete" : ""}`}>
									{current > (index + 1) || isComplete ? <span>&#10004;</span> : (index + 1)}
			...
			<div className='button-box'>
				{
					!isComplete && (<Button isLabel={false} label="confirm" buttonType="text" onButtonClick={fnNextActivity}>{current === stepperData.length ? <span>Order Confirm</span> : <span>Confirm</span>}</Button>)
				}
			</div>
			{
				isComplete &&
