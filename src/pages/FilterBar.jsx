...
const FilterBar = () => {
	...
	const getFilterList = (value) => {
		console.log(value);
		const getList = value && productData.filter(ele => ele?.title?.toLowerCase().includes(value.toLowerCase()));
		console.log(getList);
		...
		<div className='filter-box-container'>
			<div className='filter-bar'>
				...
				<div ref={searchBox} className="search-button-box">
					<Button isLabel={false} label='category' buttonType='text' onButtonClick={handleSearchButton}>
						<span>
							Category
						</span>
					</Button>
					{
						categories.length > 0 ?
							<Button isLabel={false} label='price' buttonType='text' onButtonClick={handleSearchButton}>
								<span>
									Price
								</span>
							</Button> : ""
					...
					{
						priceRange !== null ? <Button isLabel={false} label='rating' buttonType='text' onButtonClick={handleSearchButton}>
							<span>
								Rating
							</span>
						</Button> : ""
			...
			<div ref={searchList} className='drop-list' onClick={handleViewProduct}>
				{
					filterList && filterList.map((ele, i) => {
						return <Button key={ele.id} isLabel={false} label={ele.id} buttonType='search-list'><span>{ele.title}</span></Button>;
					})
				}
			</div>
			{
				showHidePopup && <div ref={popup} className="popup" >
					<Button isLabel={false} buttonType='icon' onButtonClick={handleClosePopup}>
