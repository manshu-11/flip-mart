...
const BagSlice = createSlice({
	...
	reducers: {
		...
		updateItemCount(state, action) {
			...
			const item = state.cartItem.find(productObj => productObj.productObj.id === id);
			if (item) {
		...
		removeCartItem(state, action) {
			...
			state.cartItem = state.cartItem.filter(productObj => productObj.productObj.id !== itemID);
		},
		removeSaveItem(state, action) {
			const saveItemID = action.payload;
			console.log(saveItemID);
			state.saveItem = state.saveItem.filter(productObj => productObj.productObj.id !== saveItemID);
		},
...
export const { setCartItem, setShowWarning, updateItemCount, setEmptyCart, removeCartItem, removeSaveItem, setItemSaveLater, setItemMoveToCart, setFontLoaded } = BagSlice.actions;
